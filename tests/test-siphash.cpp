...
int main (int argc, char * argv[])
{
    (void) argc;
    (void) argv;
    // test vectors from reference impl
    const uint8_t vectors_8[][8] = {
    ...
    uint8_t idx;
    for(idx = 0; idx < sizeof(key); idx++) key[idx] = idx;
    for(idx = 0; idx < sizeof(buf); idx++) buf[idx] = idx;

    for(idx = 0; idx < sizeof(buf); idx++)
    {
        i2p::crypto::Siphash<8>(h8, buf, idx, key);
        if(memcmp(h8, vectors_8[idx], 8)) 
        {
            std::cerr << "siphash<8> iteration " << (int)idx << " failed" << std::endl;
            std::cerr << "buffer: ";
            dumphex(buf, idx);
            std::cerr << "test vector: ";
            dumphex(vectors_8[idx], 8);
            std::cerr << std::flush;
            std::cerr << "value: ";
            dumphex(h8, 8);
            return 1;
        }
        i2p::crypto::Siphash<16>(h16, buf, idx, key);
        if(memcmp(h16, vectors_16[idx], 16)) 
        {
            std::cerr << "siphash<16> iteration " << (int)idx << " failed" << std::endl;
            std::cerr << "buffer: ";
            dumphex(buf, idx);
            std::cerr << "test vector: ";
            dumphex(vectors_16[idx], 16);
            std::cerr << std::flush;
            std::cerr << "value: ";
            dumphex(h8, 16);
            return 1;
