...
namespace http {
    ...
    void ShowSAMSessions (std::stringstream& s)
	{
		...
		for (auto& it: sam->ListSessions ())
		{
			s << "<a href=\"/?page=" << HTTP_PAGE_SAM_SESSION << "&sam_id=" << it->ID << "\">";
			s << it->ID << "</a><br>\r\n" << std::endl;
		}
	}
	...
	static void ShowSAMSession (std::stringstream& s, const std::string& id)
	{
		...
		auto& ident = session->local_ident;
		s << "<a href=\"/?page=" << HTTP_PAGE_LOCAL_DESTINATION << "&b32=" << ident.ToBase32 () << "\">";
		s << i2p::client::context.GetAddressBook ().ToAddress(ident) << "</a><br>\r\n";
		s << "<br>\r\n";
		s << "<b>Streams:</b><br>\r\n";
		for (const auto& it: session->sockets)
		{
			switch (it.sock_type)
			{
			case i2p::client::sam::eSocketTypeSession  : s << "session"; break;
			case i2p::client::sam::eSocketTypeStream   : s << "stream";  break;
			case i2p::client::sam::eSocketTypeAccept : s << "acceptor"; break;
			case i2p::client::sam::eSocketTypeMaster : s << "master"; break;
