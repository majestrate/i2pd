	...
	size_t GarlicRoutingSession::CreateDeliveryStatusClove (uint8_t * buf, uint32_t msgID)
	{
		...
		if (m_Owner)
		{
			...
			if (inboundTunnel)
			{
				...
				size += 4;
				// create msg
				auto msg = m_Router->CreateDeliveryStatusMsg (msgID);
				if (m_Owner)
				{
					//encrypt
			...
			switch (deliveryType)
			{
				...
				case eGarlicDeliveryTypeTunnel:
				{
					...
					if (from) // received through an inbound tunnel
					{
						...
						m_Router->SendTo (gwHash, i2p::CreateTunnelGatewayMsg (gwTunnel, msg)); // send directly
					break;
				}
