...
namespace garlic
{
		...
		std::shared_ptr<const i2p::data::RoutingDestination> destination, bool attachLeaseSet)
	{
		...
		if (!session)
		{
			session = CreateNewRoutingSession(destination, 	attachLeaseSet ? m_NumTags : 4, attachLeaseSet);
			std::unique_lock<std::mutex> l(m_SessionsMutex);
	...
	GarlicRoutingSessionPtr GarlicDestination::CreateNewRoutingSession(std::shared_ptr<const i2p::data::RoutingDestination> destination, int numTags, bool attachLeaseSet)
	{
		return std::make_shared<GarlicRoutingSession>(this, destination, numTags, attachLeaseSet);
	}
	...
	void GarlicDestination::VisitAllRoutingSessions(GarlicRoutingSessionVisitor v)
	{
		std::unique_lock<std::mutex> lock(m_SessionsMutex);
		for(auto & itr : m_Sessions) v(itr.second);
	}
