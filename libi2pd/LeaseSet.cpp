...
namespace data
{
	...
	LeaseSet::LeaseSet (RoutingSource * source, const uint8_t * buf, size_t len, bool storeLeases):
		m_Source(source),
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (ts < lease.endDate + LEASE_ENDDATE_THRESHOLD)
			{
				...
				if (m_StoreLeases)
				{
					...
					if (!m_Source->FindRouter (lease.tunnelGateway))
					{
						// if not found request it
						LogPrint (eLogInfo, "LeaseSet: Lease's tunnel gateway not found, requesting");
						m_Source->RequestDestination (lease.tunnelGateway);
					}
				}
			}
			else
