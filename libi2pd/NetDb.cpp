...
namespace data
{
	...
	void NetDb::Run ()
	{
		uint32_t lastSave = 0, lastPublish = 0, lastExploratory = 0, lastManageRequest = 0, lastDestinationCleanup = 0, lastFloodfillReseed = 0;
		while (m_IsRunning)
		{
			try
			{
  ...
  void NetDb::SetHidden(bool hide) {
	...
	{
		if(!i2p::transport::transports.ServiceIsReady()) {
			LogPrint(eLogError, "NetDB: cannot reseed from floodfill, transports not online");
			return;
		}
		if (!m_FloodfillBootstrap) {
			LogPrint(eLogWarning, "NetDB: no floodfill specified for reseed");
			return;
		}
		LogPrint(eLogInfo, "NetDB: reseeding from floodfill ", m_FloodfillBootstrap->GetIdentHashBase64());
		std::vector<std::shared_ptr<i2p::I2NPMessage> > requests;

		i2p::data::IdentHash ourIdent = i2p::context.GetIdentHash();
		i2p::data::IdentHash ih = m_FloodfillBootstrap->GetIdentHash();
	...
	void NetDb::HandleDatabaseSearchReplyMsg (std::shared_ptr<const I2NPMessage> msg)
	{
		...
		// if we are trying to bootstrap from floodfill request all routers
		if (m_FloodfillBootstrap && ! dest) {
			// request all in this response because we are bootstrapping from floodfill
			auto ih = m_FloodfillBootstrap->GetIdentHash();
			for (int i = 0; i < num; i++)
			{
				const uint8_t * router = buf + 33 + i*32;
				i2p::data::IdentHash routerIH(router);
				LogPrint(eLogInfo, "NetDB: requesting ", routerIH.ToBase64(), " for bootstrap");
				RequestDestinationFrom(router, ih, true, [&] (std::shared_ptr<RouterInfo> ri) {
						if(ri)
							LogPrint(eLogInfo, "NetDB: found ", ri->GetIdentHashBase64());
				});
			}
			return;
		}
