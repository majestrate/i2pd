...
namespace client
{
	...
	LeaseSetDestination::LeaseSetDestination (ioservice_ptr service, bool isPublic, const std::map<std::string, std::string> * params):
		m_IsRunning (false), m_Thread (nullptr),
		m_Service(service),
		m_IsPublic (isPublic),
		m_PublishReplyToken (0), m_LastSubmissionTime (0), m_PublishConfirmationTimer (*service.get()),
		m_PublishVerificationTimer (*service.get()), m_PublishDelayTimer (*service.get()), m_CleanupTimer (*service.get())
	{
	...
	std::thread * LeaseSetDestination::SpawnThread()
	{
		return new std::thread (std::bind (&LeaseSetDestination::Run, shared_from_this ()));
	}

	void LeaseSetDestination::StopService()
	{
		m_Service->stop();
	}
	...
	ClientDestination::ClientDestination (const i2p::data::PrivateKeys& keys, bool isPublic, const std::map<std::string, std::string> * params) : ClientDestination(std::make_shared<boost::asio::io_service>(), keys, isPublic, params) {}
	
	ClientDestination::ClientDestination (ioservice_ptr service, const i2p::data::PrivateKeys& keys, bool isPublic, const std::map<std::string, std::string> * params):
		LeaseSetDestination (service, isPublic, params), m_Keys (keys), m_StreamingAckDelay (DEFAULT_INITIAL_ACK_DELAY),
