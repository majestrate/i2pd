...
namespace tunnel
{
    ...
    TunnelConfig (i2p::data::RoutingDestination * source, std::vector<std::shared_ptr<const i2p::data::IdentityEx> > peers) // inbound
		{
      m_Source = source;
      CreatePeers (peers);
      m_LastHop->SetNextIdent (m_Source->GetIdentHash ());
    }

    TunnelConfig (i2p::data::RoutingDestination * source, std::vector<std::shared_ptr<const i2p::data::IdentityEx> > peers,
  ...
  protected:
    ...
    /** get routing destination that owns this tunnel */
    i2p::data::RoutingDestination * GetSource() { return m_Source; }
    const i2p::data::RoutingDestination * GetSource() const { return m_Source; }
  ...
  private:
    ...
    void CreatePeers (const Peers& peers)
    {
      ...
      m_LastHop = prev;
		...
		private:
    i2p::data::RoutingDestination * m_Source;
	...
	class ZeroHopsTunnelConfig: public TunnelConfig
	{
  	  ...
  	  const i2p::data::IdentHash& GetNextIdentHash () const { return GetSource()->GetIdentHash (); };
	    const i2p::data::IdentHash& GetLastIdentHash () const { return GetSource()->GetIdentHash (); };
