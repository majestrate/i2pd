...
namespace client
{
	...
	{
		bool matched = params && params->find(I2P_CLIENT_TUNNEL_MATCH_TUNNELS) != params->end() && params->find(I2P_CLIENT_TUNNEL_MATCH_TUNNELS)->second == "true";
		std::shared_ptr<ClientDestination> localDestination = nullptr;
		if (destination != "")
		{
			i2p::data::PrivateKeys keys;
			if (!keys.FromBase64 (destination)) return nullptr;
			localDestination = i2p::client::context.CreateNewLocalDestination (keys, true, params, matched);
		}
		else // transient
		{
			// extract signature type
			...
			localDestination = i2p::client::context.CreateNewLocalDestination (true, signatureType, params, matched);
		}
		if (localDestination)
		{
